...
bool
CAVE::Is_Square_Valid (const vector < int >&square)
{
for (auto e:square)
    if (e < 1 || e > 4)
      return false;
...
CAVE::Initialize_Cave_Map ()
{
  ...
  //keep generating pits until it doesn't lay upon wumpus
  for (int i = 0; i < 3; i++)
    {
...
void
CAVE::Print_Square (const vector < int >&rc)
{
...
void
CAVE::Initialize_Cave_Diagram ()
{ visited.insert(vector<int>{1,1});
  cave_diagram.clear ();
  int cell_rows = 5;  //subject to change: the size of a cell
  int cell_columns = 11;
  int total_rows = cell_rows * 4 + 1;
  int total_columns = cell_columns * 4 + 1;
  for (int r = 0; r < total_rows; r++) //splice colums and then rows to store the cave
    {
  ...
  for (int r = 0; r < total_rows; r += cell_rows)
    ...
    //fill in unvisited blank tiles with unknown
    for(int i=1; i<=4; i++)
    for(int j=1; j<=4; j++){
        const vector<int> v{i,j};
        if(cave_map[v].empty()){
            int row = 1 + cell_rows * (i - 1);
            int col = 3 + cell_columns * (j - 1);
            if(visited.find(v) == visited.end())
            cave_diagram[row].replace(col, 7, "unknown");
        }
    }
...
bool
CAVE::Remove_Gold (const vector < int >&rc)
{
...
bool
CAVE::Square_Contains_Living_Wumpus (const vector < int >&rc)
{
...
bool
CAVE::Square_Contains_Pit (const vector < int >&rc)
{
  if (!Is_Square_Valid (rc))
    return false;
...
bool
CAVE::Square_Contains_Gold (const vector < int >&rc)
{
  if (!Is_Square_Valid (rc))
    return false;
  if (cave_map[rc].count ("gold"))
...
bool
CAVE::Square_Contains_Ladder (const vector < int >&rc)
{
  if (!Is_Square_Valid (rc))
    return false;
...
bool
CAVE::Shoot_Square (const vector < int >&rc)
{
...
bool
CAVE::Kill_Wumpus (const vector < int >&rc)
{
  if (!Is_Square_Valid (rc))
    return false;
  ...
  if (cave_map[rc].count ("wumpus"))
    {
      cave_map[rc].erase ("wumpus");
      cave_map[rc].insert ("corpse");
      Initialize_Cave_Diagram ();
      return true;
    }
  else
